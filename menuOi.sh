    ...
    case $verifica in # case MENU
       ...
       1) while :; do
           ...
           grepi4=$(echo $response | grep -c "Transaction limit exceeded")
                 case $grepi4 in
                      1) echo -e "\e[01;32m LIMITE DE TENTATIVAS EXCEDIDO NESTE TOKEN\e[0m / \e[01;37mCONTAGEM: $i\e[0m"|pv -qL 25;;
                      *)
                 esac
           grepi5=$(echo $response | grep -c "7x1 elegibility")
                 case $grepi5 in
                      1) echo -e "\e[01;32m SERVIDOR DANDO PAU! CORRE BERG\e[0m / \e[01;37mCONTAGEM: $i\e[0m"|pv -qL 25;;
                      *)
                 esac
              done
           done
       ...
       4) tail -n +120 principal > principal1 && head -n +22 principal1 > principal2 && rm principal1 && chmod +x principal2 && ./principal2;;
